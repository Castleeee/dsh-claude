...
- Corrected live-UI diagnosis: clean-browser console evidence showed the plugin Definition `claude-code-activity` publishing Location data key `claudeCode`; DSH requires `data.key === definition.kind`, so each projection flush threw before the completed conversation snapshot could publish. The earlier Host-status attribution was disproved; Host true-to-false delivery was correct.
- Repair: the Definition now owns kind/key `claudeCode`, and the regression asserts the published key equals the Definition kind.
- Browser evidence: after rebuilding and refreshing the existing DSH URL, a completed history opened with its assistant reply and 16 activity rows, `loading:false`, `diving:false`, and no console errors. A second live turn completed without any refresh, rendered its reply and 7 activity rows, cleared `Deep diving`, and produced no console errors.
...
- Final GUI acceptance: after refreshing the user's existing DSH page, a new visible Claude Code `Hi` turn displayed its reply and exited `Deep diving` normally.
