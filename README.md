...
- DeepSeek Harness Desktop with compatible public plugin APIs. This package is currently developed against the DSH `0.1.5-rc.2` package line (DSH Desktop 2.0.7). Plugin 0.1.37 and later import symbols that do not exist on `0.1.1-rc.2`; a Host still on that line must pin `@norman-else/dsh-claude@0.1.36`.
...
- **Thinking effort** — Maps DSH's per-model reasoning effort onto Claude Code's thinking modes — `off`, `low`, `medium`, `high` (Claude Code's own default), `xhigh`, `max`, and `ultracode` — so the conversation's effort control drives extended thinking directly. Models that do not support a mode downgrade silently inside Claude Code; `ultracode` additionally turns on standing dynamic-workflow orchestration and needs an `xhigh`-capable model. Moving between modes that a running process accepts goes through one control request instead of a respawn, so the process and the context it already holds survive the change; the modes that are start-time shapes — thinking off, `ultracode`, and the CLI's own default — still rebuild, and a CLI that refuses the request falls back to the old path.
...
- **Live turn state** — While a turn runs, the footer under it says what Claude is doing right now — thinking, working through a tool result, or running a named tool with its clock. It is fed by the CLI's own progress telemetry, travels as a state rather than a transcript row, is never written to disk, and disappears with the turn it describes. Changing reasoning effort mid-turn does not disturb it.
- **Session titles** — Answers DSH's session-title request with a throwaway Haiku turn that runs against the user's own Claude Code settings, so the title is a real summary of the first message. A CLI failure reply is refused rather than accepted: an answer like `Not logged in · Please run /login` is not a title, and rejecting it leaves the session to be titled properly instead of carrying the error text.
...
- **Message queue and steering** — Accepts further messages while a turn is running, and lets a queued message be edited, removed, or steered into the turn already in flight — attachments included, because a steered message is resolved through the same code path an ordinary send uses. The CLI's own queue → run → done bookkeeping for that message is drawn as one row under the turn that received it, so a reader who typed mid-turn can see that Claude heard it. A message the CLI drops before reading it is reported as the failure it is.
...
- **Redacted activity timeline** — Displays thinking summaries (bounded at 4,000 characters, with the cut marked), tool calls and results, permission events, questions, status changes, usage, errors, and subagent activity without persisting credentials. A configured hook is drawn as one row per invocation, folded from its start to its response so a hook that holds or bends the turn is distinguishable from a slow model; a hook that fails keeps its exit code and its output.
- **Todo lists** — A `TodoWrite` call is drawn as the checklist it is — completed entries struck through, the entry in progress marked — with the collapsed row saying how far through the list the turn is, instead of printing the list as a line of JSON.
...
- **Background task tracking** — Shows running and completed Claude subagents or background tasks with task status, recent tools, and expandable activity, and lets the board end a running one: the request is checked against the board, one control request goes to the live process, and a task the CLI cannot reach leaves the board exactly as it was.
- **Context usage** — Tracks how much of the context window a session has consumed and surfaces it as a percentage in the conversation and on the session board. In the composer, a Claude session gets this plugin's own meter in place of the Host's: the Host draws its ring and composition from the request DSH assembled, and for this preset the context lives inside Claude Code, where DSH can see neither the conversation nor the tools. The plugin's meter is the same ring in the same seat, opened onto the CLI's own context report — every category it accounts for (system prompt, tool definitions, MCP tools, skills, memory files, messages), what those messages are made of (tool results dominate a real session), whether Claude Code compacts on its own and at what occupancy, and the before/after of the newest compaction, which only the boundary frame carries. Sessions of every other preset keep the Host's meter untouched; a report from a CLI too old to carry a field simply leaves that line out rather than claiming a default. While a Claude session is on screen the Host's own ring and its statistics row are hidden, and the plugin draws both readings in the seats they held: the ring is ordered straight after the model seat and straight before the submit button, and the statistics row registers into the same composer dock the Host's own does.
- **Session statistics** — The composer's turn, rate, and token pills, from what Claude Code itself reported. Turns are the session's turns, and the steps beside them are the model calls inside them — one DSH step here spans a whole Claude turn — so the pills count what they name. The rate divides output tokens by the API time those calls took, not by the turn's wall clock, which for this preset also covers tool execution; a turn that reported no calls yet simply omits the rate rather than inventing one. The token pill sums everything the CLI billed, uncached input plus cache reads, cache writes, and output, with the cache-hit share of that prompt side read the way the Host reads its own (and never rounded up to 100 when it is not). Each pill opens onto the Host's own dialog frame: session totals, model time against wall time, average first-token wait, rate, and the usage buckets with the CLI's cumulative cost.
  DSH's own accounting for these sessions is stood down along with its meter and its row, and that is also what keeps DSH's compaction out of them: the plugin reports no prompt side to DSH at all, so its pressure stays at nothing and its compaction engine never engages on a session whose context belongs to Claude Code, which compacts on its own. The plugin's own record — the sidecar rows the turn footer and the session board read — keeps the full turn totals.
...
- **Managed process lifecycle** — Keeps one live Claude process per active session, serializes turns, evicts idle processes, queues user turns FIFO when every process slot is busy, converges safely after the limit is lowered, and handles Stop, cancellation, restart, and process-tree cleanup. A cancelled turn keeps its process whenever Claude Code confirms it stopped, so the next prompt continues in that same process instead of respawning and resuming from disk.
